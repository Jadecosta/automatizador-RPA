...
from scripts.comparativo import gerar_comparativos
...
from scripts.calculos import (dias_trabalhados_13, dias_trabalhados_ferias,
                              ...
                              ferias_nao_gozadas, indenizacao_total, ferias_nao_gozadas)
import pandas as pd
import jinja2
import numpy as np
...
ALLOWED_USERNAMES = ['daniel.gomes@sead.pi.gov.br', 'tatilaaraujo@sead.pi.gov.br', 'jade.santiago@sead.pi.gov.br']
...
def comparativo_folha():
    if request.method == 'POST':
        ...
        tabelas = request.form.getlist('tabelas')
        acao = request.form.get('acao')                                    
        
        # Remove "TODOS" da lista e usa todas as tabelas disponíveis
        if "TODOS" in tabelas:
            tabelas = [t for t in tabelas_disponiveis if t != "TODOS"]
        
        try:
            con, cur = conectar_banco()  
            df_aba1, df_aba2, df_aba3, df_aba4 = gerar_comparativos(cur, mes, ano, tabelas)
            
            data_atual = datetime.now()
            base_path = f'C:/Github/App_Program/arquivos_gerados/COMPARATIVO_{data_atual.strftime("%Y%m%d%H%M%S")}'
            
            arquivos = {
                'aba1': f"{base_path}_Mes_Corrente.csv",
                'aba2': f"{base_path}_Mes_Anterior.csv",
                'aba3': f"{base_path}_Rubrica_Por_Orgao.csv",
                'aba4': f"{base_path}_Rubrica.csv"
            }

            if acao in arquivos:
                df = {'aba1': df_aba1, 'aba2': df_aba2, 'aba3': df_aba3, 'aba4': df_aba4}.get(acao)
                file_path = arquivos[acao]
                df.to_csv(file_path, index=False)
            else:
                return jsonify({'status': 'erro', 'mensagem': 'Ação não reconhecida'})

            #return jsonify({'status': 'sucesso', 'mensagem': 'Comparativo concluído com sucesso!'})
            return send_file(file_path, as_attachment=True)

        except Exception as e:
            return jsonify({'status': 'erro', 'mensagem': f'Erro ao processar os dados: {e}'})

    tabelas_disponiveis = ['TODOS', 'ADAPI', 'ADH', 'AGESPISA', 'AGRESP', 'ATIPI', 'BEP', 'CBMPI', 'CCOM', 'CENDROGAS', 'CEPM',  'CGEPI', 
    ...
    return render_template('comparativo_folha.html', tabelas=tabelas_disponiveis)
...
def calcular():
    ...
    dias_13 = dias_trabalhados_13(data_termino)
    ...
    if tipo == "Prestador de Serviço/Temporario":
        ...
        resposta = {
            ...
            'valor_ferias_proporcionais': 0.0, 'valor_terco_ferias': 0.0,
            'valor_ferias_nao_gozadas': 0.0, 'total': total
        }
    ...
    valor_ferias_nao_gozadas = ferias_nao_gozadas(base_calc, cet_calc, dias_13)
    total = indenizacao_total(valor_13, valor_ferias_prop, valor_ferias_nao_gozadas, valor_terco)

    if incluir_restituicao:
        valor_restituicao = round((liquido_calc / 30) * dias_restituir, 2)
        total += valor_restituicao
    else:
        valor_restituicao = None
